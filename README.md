...
- **Static mood beds** — 6 ambient WAVs synthesized once in-house and shipped as static files. No live procedural score, no licensed soundtrack.
...
- **Universal WorldManifest engine (v2)** — a pure, deterministic function of `(book, scenes, characters)` turns any book into a tiny planet: one place per scene laid out on a **fibonacci sphere** (lat/lon), a portal for each story-graph edge (branching-aware DAG, not just the linear chain), NPCs scheduled from each scene's `characters_present` so characters traverse the same places in the same order as the book (Rama: Ayodhya → forest → Lanka) and visibly migrate as the avatar unlocks scenes, a per-place biome inferred from mood/keywords that tints the planet's terrain in distinct regions, and a cozy palette keyed by book slug. The synthesizer itself is pure — no AI, no database, no network. The 3D canvas loads scene art + NPC portraits over the network when available (S3/CloudFront) and degrades to biome-colored procedural tiles + emoji offline, so it works against the curated seed canon either way.
...
- **Lightweight NPC life** — idle characters ring each place with a portrait billboard (when a portrait URL is available; emoji is the graceful fallback) and a phrase drawn from their `talk_examples`. They do not gate progress; they are warmth.
...
- **Music is a static synthesized mood bed.** Six mood WAVs (serene/dramatic/somber/joyful/sacred/mysterious) generated once by `lib/audio/proceduralWav.ts` and shipped as static files — not a live procedural score or a film soundtrack.
